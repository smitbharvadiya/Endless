...
function setup() {
  ...
  startGame = createSprite(width/2,height/2,50,50);
...
function draw() {
  ...
  if (gameState === START) {
    ...
    camera.position.y = iornMan.y - 190
...
//iornMan.setCollider("rectangle",250,200,200,800);
  ...
  if (gameState === START) {
    ...
    fill("black");
    textSize(30);
    text("CHHOTA BHEEM", width/3.2,height/6);
    stroke("black");
    fill("black");
    textSize(25);
    text("Press Start Button To Play", width/3.6,height/4);
  }
  ...
  if (gameState === PLAY || gameState === END) {
    ...
    fill("black");
    textSize(20);
    text("Score: " + score, width/2.3, iornMan.y - 330)
  }
  ...
  if (gameState === END) {
   //camera.y
    ...
    fill(1);
    textSize(50);
    text("GAME OVER!☠", width/4.5,camera.y - 15 );
...
function spawnClouds() {
  ...
  if (frameCount % 15=== 0) {
    ...
    cloud.x = Math.round(random(10,370));
